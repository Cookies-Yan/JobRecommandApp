package com.recommand.job.servlet;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.recommand.job.db.MySQLConnection;
import com.recommand.job.entity.LoginRequestBody;
import com.recommand.job.entity.LoginResponseBody;
...
public class LoginServlet extends HttpServlet {
    ...
    //Get 只是为了测试是不是login， POST才是真正的login操作
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ObjectMapper mapper = new ObjectMapper();
        HttpSession session = request.getSession(false); //其实是create
        LoginResponseBody loginResponseBody;
        if (session != null) {
            MySQLConnection connection = new MySQLConnection();
